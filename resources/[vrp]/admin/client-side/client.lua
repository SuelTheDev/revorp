...
--[[Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(0,121) then
			TriggerServerEvent("cds:corridas")
		end
	end
end)]]
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler('deletarveiculo',function(vehicle)
	if IsEntityAVehicle(vehicle) then
		TriggerServerEvent("vrp_garages_id",VehToNet(vehicle),GetVehicleEngineHealth(vehicle),GetVehicleBodyHealth(vehicle),GetVehicleFuelLevel(vehicle))
...
AddEventHandler("syncdeleteveh",function(vehicle)
...
AddEventHandler("vehtuning",function()
    ...
    if IsEntityAVehicle(vehicle) then
        ...
        SetVehicleNumberPlateText(vehicle,"SHELBY")
