        ...
        if IsPedArmed(ped, 6) then
            ...
            if IsPedShooting(ped) then
                ...
                local infos = {
                    cds = abcd,
                    codigo = '10-71',
                    ocorrencia = 'Disparo de arma de fogo.',
                    rua = streetName,
...
AddEventHandler('notificacao',function(x,y,z,user_id,infos)
    ...
    --if distance <= 2100 then
        if not DoesBlipExist(blips[user_id]) then
            ...
            -- TriggerEvent('feijonts:NotificacaoPolicia', 'police', {["code"] = '10-76', ["name"] = 'Disparo de arma de fogo.', ["loc"] = 'teste'}, 8000)
            TriggerEvent('feijonts:NotificacaoPolicia', 'police', {["code"] = infos.codigo, ["name"] = infos.ocorrencia, ["loc"] = infos.rua}, infos.tempoNotificacao)
...
AddEventHandler('cone',function(nome)
	...
	if nome ~= "d" then
		...
		Entity(cone).state:set('checked', 1, true)
...
AddEventHandler('barreira',function(nome)
	...
	if nome ~= "d" then
		...
		Entity(barreira).state:set('checked', 1, true)
...
AddEventHandler('spike',function(nome)
	...
	if nome ~= "d" then
		...
		Entity(spike).state:set('checked', 1, true)
