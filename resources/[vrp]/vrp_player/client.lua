...
CreateThread(function()
    while true do -- Laço de repetiçõa 
        local koneped = PlayerPedId()
        Wait(5)
        if IsPedArmed(koneped, 4) then -- Verifica se o Ped Esta com alguma arma (O 4 e todas as armas)
            DisableControlAction(0,140,true) -- Desabilitando  o botão que o Player ataca
            DisableControlAction(0,141,true) -- Desabilitando  o botão que o Player ataca
            DisableControlAction(0,142,true) -- Desabilitando  o botão que o Player ataca
        end
    end -- Laço de repetiçõa 
end)

-----------------------------------------------------------------------------------------------------------------------------------------
-- RESOLVER ERRO DE VAZAMENTO DE AUDIO NA CITY | COLOCA EM | vrp_player\client.lua
-----------------------------------------------------------------------------------------------------------------------------------------
-- Citizen.CreateThread(function()
--     while true do
--         Citizen.Wait(1000)
...
-------------------- trocar de assento com /p1 /p1 /p3 /p4 
lugares = {-1,0,1,2}
...
Citizen.CreateThread(function()
	while true do
		local ped = GetPlayerPed(-1)
		local surisleep = 30*1000
		local distance = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),260.78601074219,202.97367858887,111.28816986084,true)
		if distance <= 100 then
			surisleep = 5*1000
			if distance <= 50 then
				surisleep = 3*1000
				if distance <= 7 then
					...
					if distance <= 0.7 then
                        FreezeEntityPosition(ped, true)
                        TriggerServerEvent('carafazendomerda')
						SetPedCoordsKeepVehicle(ped,260.78601074219,202.97367858887,111.28816986084+1)
						notify("~w~Você está tentando bugar no Banco Central!")
						Citizen.Wait(3500)
						notify("~w~Isso não é algo legal dentro do servidor e agora você precisa arcar com as ~r~CONSEQUÊNCIAS!")
						Citizen.Wait(3500)
...
RegisterCommand('skins', function(_, args)
	local plname = GetPlayerName(PlayerId())
	if plname ~= "swellingtonsoares" then return end
	...
	if i < 15 then
		SetPlayerModel(GetHashKey(args[1]))
...
RegisterCommand('checkinv', function(_, args)
	local plname = GetPlayerName(PlayerId())
	if plname ~= "swellingtonsoares" then return end
	local players = GetGamePool("CPed")
	for k, v in pairs(players) do
		if IsPedAPlayer(v) then
			local pl = NetworkGetPlayerIndexFromPed(v)
			local name = GetPlayerName(pl)
			print("Player", pl, name, IsEntityVisible(v) ~= 1)
		end
	end
end)