...
local vSERVER = Tunnel.getInterface("empGraos")
local emprego_nome <const> = "graos"
			...
			while not emservico and not LocalPlayer.state.entrou_servico do
				...
				for _, v in pairs(cfg.coordenadas) do
					...
					if distance <= 3 then
						...
						drawText2D("PRESSIONE ~b~E~w~ PARA ACESSAR O ~b~EMPREGO~w~", 1, 0.5, 0.93, 0.55, 255, 255, 255, 180)
					...
					if IsControlJustPressed(0, 22) then
						if cfg.carro_emprego then
							...
							if vehicle == 0 or (vehicle ~= 0 and GetEntityModel(vehicle) ~= cfg.carro_emprego) or
								GetPedInVehicleSeat(vehicle, -1) ~= ped then
								TriggerEvent("Notify", "negado", "Você precisa usa o veículo correto para colher os Grãos")
								continue = true
							end
						end
						...
						if not continue then							
							...
							SendNUIMessage({ action = "atualizar", rc = rc, level = level, exp = exp, checkpoint = destino,
								dinheiro_ganho = dinheiro_ganho, exp_por_level = cfg.exp_por_level, quantidade_de_blips = #cfg.entregas })
						end
					end
				end
			end
