...
function spawnVehicle(vehicleInfo, coords, h)
	if func.checkAuth() then
		...
		local plate = vRP.getRegistrationNumber()
		func.spawnVeh(mhash, coords, h, plate, vehicleInfo)
		local vehicle = false
...
RegisterNUICallback("get-tuning", function(data, cb)
	if data and data.vehicle then
		...
		if tuning then
			...
			tunagem = { motor = engine, freios = brakes, transmissao = transmission, suspensao = suspention, blindagem = armour,
				turbo = turbo }
		end
		cb(tunagem)
	end
...
AddEventHandler('spawnarveiculo2', function(name, x)
	if not x or x and x > 100 then return end
	...
	if HasModelLoaded(mhash) then
		...
		Entity(nveh).state:set('checked', '1', true)
...
RegisterNetEvent("nation_garages:client:triggerVehicleShowGps", function(model, netId, pos)
	local blipId = 304
	local blipColor = 46
	local vehName = GetDisplayNameFromVehicleModel(model)
	local blip
	if NetworkDoesEntityExistWithNetworkId(netid) then
		blip = AddBlipForEntity(NetToVeh(netId))
	else
