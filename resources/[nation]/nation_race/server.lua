...
function func.getValor()
	...
	if dinheiro then
		if vRP.request(source,"Você deseja pagar <b>R$ "..vRP.format(dinheiro).."</b> sujo(s) para entrar na corrida?", 30) then
			if vRP.tryGetInventoryItem(user_id,"dinheirosujo",dinheiro) then
				local nickname = func.getNickName(user_id)
				table.insert(corredores, { corredor = user_id })
				fclient.updateCorredor(source,true)
				...
				TriggerClientEvent("Notify",source,"negado","Você não possui <b>R$ "..vRP.format(dinheiro).." sujo(s).", 5000)
			end
		end
	end
	return false
...
function func.checkReward(tempofinal,index)
	...
	if rank == 1 then
		...
		TriggerClientEvent("Notify",source,"sucesso","Parabéns, você recebeu R$ <b>"..vRP.format(reward).."</b> sujo(s).", 5000)
	end
