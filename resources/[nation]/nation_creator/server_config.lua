...
if multiCharacter then
    vRP._prepare("nation_creator/createAgeColumn","ALTER TABLE vrp_users ADD IF NOT EXISTS age INT(11) NOT NULL DEFAULT 20")
    vRP._prepare("nation_creator/update_user_first_spawn","UPDATE vrp_users SET name2 = @firstname, name = @name, age = @age WHERE id = @user_id")
    vRP._prepare("nation_creator/create_characters","INSERT INTO vrp_users(steam,name,name2) VALUES(@steam,@name,@name2)")
...
function func.getCharsInfo()
    ...
    for k,v in ipairs(data) do
        ...
        info.chars[k] = {
            name = v.name.." "..v.name2, age = v.age.." anos", bank = "$ "..format(v.bank), clothes = clothes,
            registration = v.registration, phone = v.phone, user_id = v.id, id = "#"..v.id, gender = gender, char = char
        }
    end
...
function getUserMaxChars(source)
    ...
    if infos and infos[1] and infos[1].chars then
        return infos[3].chars -- máximo de chars para criar
    end
    return 1
end
...
function func.tryCreateChar()
    ...
    if #data < getUserMaxChars(source)  then -- limite de personagens
        vRP.execute("nation_creator/create_characters",{ steam = steam, name = "", name2 = "" })
...
RegisterCommand('resetchar',function(source, args) -- COMANDO DE ADMIN PARA RESETAR PERSONAGEM
    if func.checkPermission({"admin.permissao", "mod.permissao", "Admin", "admin.permissao"}, source) then
        if args[1] then 
