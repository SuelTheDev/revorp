$(function() {
  ...
  function init() {
	...
	$('.category_item').show(function() {
		...
		function showArma() {
			$('.item-item[category="armamentos"]').show();
			$('.item-item[category="armamentos"]').css('transform', 'scale(1)');
		}
  ...
  function sendData(name, data) {
	$.post("http://nation_arsenal/" + name, JSON.stringify(data), function(
	  datab
	) {
	  if (datab != "ok") {
		console.log(datab);
