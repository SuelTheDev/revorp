  ...
  $( ".allstar__completar" ).click(function() {
    if (!counting) {
      perc_new = 100 - Math.round(datafuel);
        ...
        completar = !completar;
        maxFuel = 99
        $('.modal span').text("Aceita abastecer o tanque com R$"+perc_new);  
        $('.modal').fadeIn(300);
        ...
        if (price+Math.round(datafuel) <= 100) {
          if (money >= price) {
            ...
            $.post('http://nation_fuel/notifytext', JSON.stringify({ text: texto }));
              ...
              var texto = "O maximo de gasto para encher o tanque é <b>R$"+(100-Math.round(datafuel))+"</b>!"
              $.post('http://nation_fuel/notifytext', JSON.stringify({ text: texto }));
            }
        } else {
          var texto = "Somente valores entre <b>$1</b> e <b>R$100</b>, o valor atual é <b>$"+Math.round($("#amount").val())+"</b>!"
          $.post('http://nation_fuel/notifytext', JSON.stringify({ text: texto }));
      }
    }
  });
