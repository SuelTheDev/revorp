	...
	if user_id and price > 0 then
		if vRP.tryPayment(user_id,price) then
			if galao then
				...
				TriggerClientEvent("Notify",source,"sucesso","Pagou <b>R$"..vRP.format(price).." dólares</b> pelo <b>Galão</b>.")
			else
				TriggerClientEvent("Notify",source,"sucesso","Pagou <b>R$"..vRP.format(price).." dólares</b> em combustível.")
			end
		else
