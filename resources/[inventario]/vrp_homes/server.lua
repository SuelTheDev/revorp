...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookbaucasas = "https://discord.com/api/webhooks/985246175349313556/J-lJcN529OpX0Xj_JlLx-ME9reM0Sj8lozwn2wrRr8lRIUHilIa5knp6KHJd1z2mIBgB"
	...
	if user_id then
		...
		elseif args[1] == "tax" and homes[tostring(args[2])] then
			...
			if ownerHomes[1] then
				--if not vRP.hasGroup(user_id,"Platina") and not vRP.hasGroup(user_id,"Diamante") and not vRP.hasGroup(user_id,"Ouro") then
...
local function RequestVault(house)
	return house and homes[house] or nil
end

RegisterNetEvent("dPN:inventario:RequestVault")
AddEventHandler("dPN:inventario:RequestVault",function(user_id,house)
	TriggerEvent('dPN:inventario:returAmountVault', user_id, RequestVault(house))
end)

exports("requestVault", RequestVault)