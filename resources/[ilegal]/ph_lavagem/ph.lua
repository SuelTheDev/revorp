			...
			if item == g.item then
				if not inProgress[src] then
					...
					if item == "lavar" then
						...
						if vRP.getInventoryItemAmount(user_id,"notafiscalfalsa") >= 1 then
					    if vRP.getInventoryItemAmount(user_id,"dinheirosujo") >= parseInt(valor) then
							...
							vRP.tryGetInventoryItem(user_id,"notafiscalfalsa",1)

							vRP.giveInventoryItem(user_id,"energetico",1)
...
function src.checkPermission()
	...
	if vRP.hasPermission(user_id,"bahamas.permissao") or vRP.hasPermission(user_id,"bahamas.permissao") then
		return true
	end
end