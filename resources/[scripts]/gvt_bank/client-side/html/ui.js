...
const makePost = () => {
    $.post(`http://gvt_bank/requestInitial`, JSON.stringify({}), (data) => {
        ...
        $('#multas strong').html(`$ ${addCommas(data.multas)}`);
        $('.sc-saldo-card').html(`$ ${addCommas(data.bank)}`);
    });
}
...
let _post = function (event, data, cb) {
    $.post(`http://gvt_bank/${event}`, JSON.stringify((data === undefined) ? {} : data), (cb === undefined) ? () => { } : cb);
}
