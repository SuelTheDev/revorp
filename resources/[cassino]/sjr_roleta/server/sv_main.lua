...
function getPlayerChips(source)
    ...
    local chips = vRP.getInventoryItemAmount(user_id,Config.ItemName)
    return chips -- DO NOT FORGET TO EDIT THIS WITH YOUR SERVER GETTER
end
...
function giveChips(source, amount)
    ...
    local user_id = vRP.getUserId(source)
    if amount > 0 then
        vRP.giveInventoryItem(user_id,Config.ItemName,amount,true)
    end
end
...
function removeChips(source, amount)
    ...
    local user_id = vRP.getUserId(source)
    if amount > 0 then
        vRP.tryGetInventoryItem(user_id,Config.ItemName,amount,true)
    end
end
