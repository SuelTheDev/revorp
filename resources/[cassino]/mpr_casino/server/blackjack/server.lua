...
function bridge.paymentt(amount)
	local source = source
	local user_id = vRP.getUserId(source)
	print(amount)
	if user_id then
		if vRP.tryGetInventoryItem(user_id,"ficha",amount) then
			return true			
		else
			return false
		end
...
function GiveMoney(source, money)
	local user_id = vRP.getUserId(source)
	print('CHAMOU', money)
	if user_id then
			vRP.giveInventoryItem(user_id,"ficha",parseInt(money))
	end
end
											...
											if tableTracker[tostring(v.player)] == nil then
												...
												if move == "hit" then
													...
													if handValue(v.hand) == 21 then
														-- PlayDealerAnim(index, "anim_casino_b@amb@casino@games@blackjack@dealer", "female_dealer_focus_player_0".. 5-v.seat .."_idle_outro")
														-- Wait(1500)
														DebugPrint("TABLE "..index..": "..GetPlayerName(v.player):upper().." Fez 21")
														-- v.player_in = false
														PlayDealerSpeech(index, "MINIGAME_BJACK_DEALER_"..handValue(v.hand))
														break
													elseif handValue(v.hand) > 21 then
														...
														-- Wait(1500)
														TriggerClientEvent("BLACKJACK:GameEndReaction", v.player, "bad")
														DebugPrint("TABLE "..index..": "..GetPlayerName(v.player):upper().." Faliu")
														v.player_in = false
												...
												elseif move == "double" then
													...
													elseif handValue(v.hand) > 21 then
														...
														-- Wait(1500)
														TriggerClientEvent("BLACKJACK:GameEndReaction", v.player, "bad")
														DebugPrint("TABLE "..index..": "..GetPlayerName(v.player):upper().." Falei")
														v.player_in = false
														PlayDealerSpeech(index, "MINIGAME_BJACK_DEALER_PLAYER_BUST")
													else
														-- Wait(2000)
								...
								if v.player_in == true and handValue(v.hand) < handValue(dealerHand) and handValue(dealerHand) <= 21 then -- LOSE
									if v.splitHand then
										if handValue(v.splitHand) < handValue(dealerHand) and handValue(dealerHand) <= 21 then -- LOSE
											TriggerClientEvent("BLACKJACK:GameEndReaction", v.player, "bad")
											DebugPrint("TABLE "..index..": "..GetPlayerName(v.player):upper().." Perdeu")
											v.player_in = false
										end
									end
									TriggerClientEvent("BLACKJACK:GameEndReaction", v.player, "bad")
