...
Citizen.CreateThread(function()
	while true do
		...
		if distance <= 1.1 and not andamento then
			sleep = 5
			drawTxt("PRESSIONE  ~r~E~w~  PARA ~b~HACKEAR ~w~O ~y~SISTEMA DE SEGURANÇA",4,0.5,0.90,0.50,255,255,255,180)
		...
		if andamento and tipo == 1 then
			drawTxt("FALTAM ~g~"..segundos.." SEGUNDOS ~w~PARA TERMINAR DE HACKEAR O ~y~SISTEMA DE SEGURANÇA",4,0.5,0.90,0.50,255,255,255,180)
		elseif andamento and tipo == 2 then
			drawTxt("FALTAM ~g~"..segundos.." SEGUNDOS ~w~PARA TERMINAR DE ~y~ROUBAR AS JOIAS",4,0.5,0.90,0.50,255,255,255,180)
		end
		Citizen.Wait(sleep)
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		for _,v in pairs(locais) do
			...
			if distance <= 1.1 and not andamento then
				sleep = 5
				drawTxt("PRESSIONE  ~r~E~w~  PARA ~y~ROUBAR AS JOIAS",4,0.5,0.90,0.50,255,255,255,180)
				if IsControlJustPressed(0,38) and not IsPedInAnyVehicle(ped) then
...
AddEventHandler("iniciandojewelry",function(x,y,z,h,sec,mod,status)
	...
	if status then
		...
		SetCurrentPedWeapon(PlayerPedId(),true)
