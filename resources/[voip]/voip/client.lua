...
local function ExitRadio()
  ...
  _info.radioChannel = ""
  if isInRadio then
    TriggerEvent("Notify","sucesso","Rádio desativado.",5000)
  ...
  function(frequency, cb)
    if frequency ~= nil and #frequency > 0 then
      if string.lower(frequency) ~= _info.radioChannel then
        ...
        TriggerEvent("Notify","sucesso","Você já está ná radio" .. " " .. frequency,5000)
...
RegisterCommand('radio',function(source,args,rawCommand)
    if vSERVER.checkRadio() then
      ...
      TriggerEvent("Notify","sucesso","Você não possui um radio.</b>.",5000)
